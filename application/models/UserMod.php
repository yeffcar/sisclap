...
class UserMod extends CI_Model {
	...
	public function get_user_clap($whereportion, $getStr = false){
		$query = $this->db->query("SELECT * FROM `datos_clap`, `user` WHERE $whereportion");
		if ($getStr) {
	...
	public function get_user_lider_clap($whereportion, $fromportion="`datos_clap`, `datos_lideres`, `user`"){
		$query = $this->db->query("SELECT * FROM $fromportion WHERE $whereportion");
		if ($query->num_rows() > 0)
		{
			return $query->result_array(); 
		}
